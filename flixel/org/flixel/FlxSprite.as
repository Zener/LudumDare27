package org.flixel
{
	...
	public class FlxSprite extends FlxObject
	{
		...
		*/
		...
		public function play(AnimName:String,Force:Boolean=false):void
		{
			...
			while(i < l)
			{
				if(_animations[i].name == AnimName)
				{
					...
					// [AOC] Refresh drawing frame's size and force an instant render
					var newWidth:int = _curAnim.framesBounds[0].width;
					var newHeight:int = _curAnim.framesBounds[0].height;
					framePixels = new BitmapData(newWidth, newHeight, true, 0x00FF0000);
					calcFrame();
					
					// [AOC] Try to correct position with new width and height
					offset.x = (newWidth - width)/2;
					offset.y = (newHeight - height)/2;
		...
		{
			if(_curAnim == null || !_curAnim.useFrameBounds) {
				...
				// [AOC] Style
				// TODO!! Handle flipped sprites
				// TODO!! Handle reversed sprites
				var frameBounds:Rectangle = _curAnim.framesBounds[_curFrame] as Rectangle;
				_flashRect.x = frameBounds.x;
				_flashRect.y = frameBounds.y;
				_flashRect.width = frameBounds.width;
				_flashRect.height = frameBounds.height;
				
				// Test - update width and frameWidth - BEFORE the copyPixels
				// Disabled for now
				/*
				...
				*/
				...
				// Now, copy the new frame
				if(_flipped && (_facing == LEFT)) {
					framePixels.copyPixels(_pixels, _flashRect, _flashPointZero);
					
					var newPixels:BitmapData = new BitmapData(framePixels.width, framePixels.height, true, 0x00FF0000); // create a new temp bitmap with the same size as the original
					var mtx:Matrix = new Matrix(); // make a new matrix for flipping
					mtx.scale(-1, 1); // flip sprite in the x dimension and keep y the same. You'll change this to mtx.scale(1,-1).
					mtx.translate(_flashRect.width, 0); // move the flipped sprite to correct the scaling
					newPixels.draw(framePixels, mtx); // draw the original bitmap to the temp bitmap applying the matrix
					framePixels.dispose();
					framePixels = newPixels;
				} else {
					framePixels.copyPixels(_pixels,_flashRect,_flashPointZero);
				}
		 ...
		 * 
		 * @param	Name		What this animation should be called (e.g. "run").
		 * @param	FramesID	The prefix of the frames in the XML file.
		 * @param	Frames		An Starling-formatted XML containing the info of the animation
		 * @param	FrameRate	The speed in frames per second that the animation should play at (e.g. 40 fps).
		 * @param	Looped		Whether or not the animation is looped or just plays once.
		 */
		public function addAnimationFromXML(Name:String, FramesID:String, Frames:XML, FrameRate:Number=0, Looped:Boolean=true):void
		{
			// Parse XML and store index, pos and size for each frame
			var frames:Array = [];
			var framesBounds:Array = [];
			var frameName:String = "";
			for each(var frameData:XML in Frames.SubTexture) {
				// Only add it if the frame's name contains the FramesID
				frameName = frameData.@name;
				if(frameName.search(FramesID) != -1) {
					// Frame index
					frames.push(frames.length);
					
					// Frame dimensions
					framesBounds.push(new Rectangle(frameData.@x, frameData.@y, frameData.@width, frameData.@height));
				}
			}
			
			// Create anim - only if frames list is bigger than 0.
			if(frames.length > 0) {
				var anim:FlxAnim = new FlxAnim(Name, frames, FrameRate, Looped);
				anim.framesBounds = framesBounds;
				anim.useFrameBounds = true;
				_animations.push(anim);
			}
			
			/*
			for(var i:int = 0; i < frames.length; i++) {
				trace("Frame " + frames[i] + "/" + frames.length + ": x=" + framesBounds[i].x + ", y=" + framesBounds[i].y + ", width=" + framesBounds[i].width + ", height=" + framesBounds[i].height);
			}
			*/
		}
		
		/**
		 * Get one of the animations added to this sprite.
		 * @param _name The name of the animation to obtain.
		 * @return The FlxAnim with the given name. Null if no anim with the given name was assigned to this sprite.
		 */
		public function getAnimation(_name:String) : FlxAnim
		{
			for(var i:int = 0; i < _animations.length; i++) {
				if((_animations[i] as FlxAnim).name == _name) return _animations[i] as FlxAnim;
			}
		...
		public function set scaleX(_val:Number) : void { scale.x = _val; }
		public function get scaleY() : Number { return scale.y; }
		public function set scaleY(_val:Number) : void { scale.y = _val; }
	}
}